//===-- CPEN211BranchSelector.cpp - Emit long conditional branches
//---------===//
//
...
namespace {
class CPEN211BSel : public MachineFunctionPass {
...
public:
  static char ID;
  CPEN211BSel() : MachineFunctionPass(ID) {}
  ...
  StringRef getPassName() const override { return "CPEN211 Branch Selector"; }
};
char CPEN211BSel::ID = 0;
} // namespace
...
unsigned CPEN211BSel::measureFunction(OffsetVector &BlockOffsets,
...
bool CPEN211BSel::expandBranches(OffsetVector &BlockOffsets) {
  ...
  //     if (MI->getOpcode() != CPEN211::JCC && MI->getOpcode() != CPEN211::JMP) {
  //       continue;
  //     }
  ...
  //      BranchDistance = BlockDistance - MBBStartOffset;
  ...
  //     if (MI->getOpcode() == CPEN211::JCC && std::next(MI) != EE) {
  ...
  //     MI = BuildMI(*MBB, MI, dl, TII->get(CPEN211::Bi)).addMBB(DestBB);
  //     InstrSizeDiff += TII->getInstSizeInBytes(*MI);
...
bool CPEN211BSel::runOnMachineFunction(MachineFunction &mf) {
  MF = &mf;
  TII =
...
FunctionPass *llvm::createCPEN211BranchSelectionPass() {
  return new CPEN211BSel();
}
