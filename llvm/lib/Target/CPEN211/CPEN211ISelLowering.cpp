...
CPEN211TargetLowering::CPEN211TargetLowering(const TargetMachine &TM,
  ...
  //********************************************************************
  //********************************************************************
  // TODO (for Vincent): this is just straight up unimplemented for now!
  //********************************************************************
  //********************************************************************
  //********************************************************************
  ...
  // TODO (for Vincent):L add libcalls?
  ...
  // TOOD (for Vincent): add floating point support?
  // if (STI.hasHWMult16()) {
  //  const struct {
  //    const RTLIB::Libcall Op;
  //    const char *const Name;
  ...
  //    setLibcallName(LC.Op, LC.Name);
  //  }
  //  // setLibcallCallingConv(RTLIB::MUL_I64, CallingConv::CPEN211_BUILTIN);
  //}

  // TODO (for Vincent): how to deal with floating point?
  // Several of the runtime library functions use a special calling conv
  // setLibcallCallingConv(RTLIB::UDIV_I64, CallingConv::CPEN211_BUILTIN);
  // setLibcallCallingConv(RTLIB::UREM_I64, CallingConv::CPEN211_BUILTIN);
  // setLibcallCallingConv(RTLIB::SDIV_I64, CallingConv::CPEN211_BUILTIN);
  // setLibcallCallingConv(RTLIB::SREM_I64, CallingConv::CPEN211_BUILTIN);
  // setLibcallCallingConv(RTLIB::ADD_F64, CallingConv::CPEN211_BUILTIN);
  // setLibcallCallingConv(RTLIB::SUB_F64, CallingConv::CPEN211_BUILTIN);
  // setLibcallCallingConv(RTLIB::MUL_F64, CallingConv::CPEN211_BUILTIN);
  // setLibcallCallingConv(RTLIB::DIV_F64, CallingConv::CPEN211_BUILTIN);
  // setLibcallCallingConv(RTLIB::OEQ_F64, CallingConv::CPEN211_BUILTIN);
  // setLibcallCallingConv(RTLIB::UNE_F64, CallingConv::CPEN211_BUILTIN);
  // setLibcallCallingConv(RTLIB::OGE_F64, CallingConv::CPEN211_BUILTIN);
  // setLibcallCallingConv(RTLIB::OLT_F64, CallingConv::CPEN211_BUILTIN);
...
bool CPEN211TargetLowering::shouldAvoidTransformToShift(EVT VT,
...
bool CPEN211TargetLowering::isLegalICmpImmediate(int64_t Immed) const {
  // making this the default?
  return Immed >= -32 && Immed < 32;
  // if (CPEN211NoLegalImmediate)
  //   return Immed >= -32 && Immed < 32;
  // return TargetLowering::isLegalICmpImmediate(Immed);
}

//===----------------------------------------------------------------------===//
//                       CPEN211 Inline Assembly Support
//===----------------------------------------------------------------------===//

/// getConstraintType - Given a constraint letter, return the type of
/// constraint it is for this target.
TargetLowering::ConstraintType
CPEN211TargetLowering::getConstraintType(StringRef Constraint) const {
...
CPEN211TargetLowering::getRegForInlineAsmConstraint(
...
#include "CPEN211GenCallingConv.inc"
...
static void AnalyzeRetResult(CCState &State,
                             const SmallVectorImpl<ISD::InputArg> &Ins) {
  State.AnalyzeCallResult(Ins, RetCC_CPEN211);
}

static void AnalyzeRetResult(CCState &State,
                             const SmallVectorImpl<ISD::OutputArg> &Outs) {
  State.AnalyzeReturn(Outs, RetCC_CPEN211);
}
...
SDValue CPEN211TargetLowering::LowerFormalArguments(
...
SDValue
CPEN211TargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  // CPEN211 target does not yet support tail call optimization.
  isTailCall = false;
...
SDValue CPEN211TargetLowering::LowerCCCArguments(
    SDValue Chain, CallingConv::ID CallConv, bool isVarArg,
    const SmallVectorImpl<ISD::InputArg> &Ins, const SDLoc &dl,
    SelectionDAG &DAG, SmallVectorImpl<SDValue> &InVals) const {
...
bool CPEN211TargetLowering::CanLowerReturn(
...
SDValue
CPEN211TargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
                                   bool isVarArg,
                                   const SmallVectorImpl<ISD::OutputArg> &Outs,
...
SDValue CPEN211TargetLowering::LowerCCCCallTo(
...
SDValue CPEN211TargetLowering::LowerBR_CC(SDValue Op, SelectionDAG &DAG) const {
  ...
  return DAG.getNode(CPEN211ISD::BR_CC, dl, Op.getValueType(), Chain, Dest,
                     TargetCC, Flag);
}
...
SDValue
CPEN211TargetLowering::getReturnAddressFrameIndex(SelectionDAG &DAG) const {
  MachineFunction &MF = DAG.getMachineFunction();
  CPEN211MachineFunctionInfo *FuncInfo =
      MF.getInfo<CPEN211MachineFunctionInfo>();
...
SDValue CPEN211TargetLowering::LowerRETURNADDR(SDValue Op,
                                               SelectionDAG &DAG) const {
...
SDValue CPEN211TargetLowering::LowerFRAMEADDR(SDValue Op,
                                              SelectionDAG &DAG) const {
  MachineFrameInfo &MFI = DAG.getMachineFunction().getFrameInfo();
  ...
  unsigned Depth = Op.getConstantOperandVal(0);
  SDValue FrameAddr =
      DAG.getCopyFromReg(DAG.getEntryNode(), dl, CPEN211::R4, VT);
  while (Depth--)
...
SDValue CPEN211TargetLowering::LowerVASTART(SDValue Op,
                                            SelectionDAG &DAG) const {
  MachineFunction &MF = DAG.getMachineFunction();
  CPEN211MachineFunctionInfo *FuncInfo =
      MF.getInfo<CPEN211MachineFunctionInfo>();
...
SDValue CPEN211TargetLowering::LowerJumpTable(SDValue Op,
                                              SelectionDAG &DAG) const {
...
bool CPEN211TargetLowering::getPostIndexedAddressParts(
...
bool CPEN211TargetLowering::isTruncateFree(Type *Ty1, Type *Ty2) const {
...
bool CPEN211TargetLowering::isTruncateFree(EVT VT1, EVT VT2) const {
...
bool CPEN211TargetLowering::isZExtFree(Type *Ty1, Type *Ty2) const {
  // CPEN211 implicitly zero-extends 8-bit results in 16-bit registers.
  return false && Ty1->isIntegerTy(8) && Ty2->isIntegerTy(16);
}

bool CPEN211TargetLowering::isZExtFree(EVT VT1, EVT VT2) const {
  // CPEN211 implicitly zero-extends 8-bit results in 16-bit registers.
...
MachineBasicBlock *
CPEN211TargetLowering::EmitShiftInstr(MachineInstr &MI,
