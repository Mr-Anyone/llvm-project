//===-- CPEN211ISelDAGToDAG.cpp - A dag to dag inst selector for CPEN211
//----===//
//
...
//===----------------------------------------------------------------------===//
//
// This file defines an instruction selector for the CPEN211 target.
//
//===----------------------------------------------------------------------===//
...
struct CPEN211ISelAddressMode {
  ...
  CPEN211ISelAddressMode() = default;
  ...
  LLVM_DUMP_METHOD void dump() {
    errs() << "CPEN211ISelAddressMode " << this << '\n';
...
///
namespace {
class CPEN211DAGToDAGISel : public SelectionDAGISel {
public:
  CPEN211DAGToDAGISel() = delete;

  CPEN211DAGToDAGISel(CPEN211TargetMachine &TM, CodeGenOptLevel OptLevel)
      : SelectionDAGISel(TM, OptLevel) {}

private:
  bool MatchAddress(SDValue N, CPEN211ISelAddressMode &AM);
  bool MatchWrapper(SDValue N, CPEN211ISelAddressMode &AM);
  bool MatchAddressBase(SDValue N, CPEN211ISelAddressMode &AM);
...
class CPEN211DAGToDAGISelLegacy : public SelectionDAGISelLegacy {
public:
  static char ID;
  CPEN211DAGToDAGISelLegacy(CPEN211TargetMachine &TM, CodeGenOptLevel OptLevel)
      : SelectionDAGISelLegacy(
            ID, std::make_unique<CPEN211DAGToDAGISel>(TM, OptLevel)) {}
};
...
/// CPEN211-specific DAG, ready for instruction scheduling.
///
FunctionPass *llvm::createCPEN211ISelDag(CPEN211TargetMachine &TM,
                                         CodeGenOptLevel OptLevel) {
  return new CPEN211DAGToDAGISelLegacy(TM, OptLevel);
}
...
bool CPEN211DAGToDAGISel::MatchAddressBase(SDValue N,
                                           CPEN211ISelAddressMode &AM) {
  // Is the base register already occupied?
  if (AM.BaseType != CPEN211ISelAddressMode::RegBase || AM.Base.Reg.getNode()) {
    // If so, we cannot select it.
    return true;
  }

  // Default, generate it as a register.
  AM.BaseType = CPEN211ISelAddressMode::RegBase;
  AM.Base.Reg = N;
  return false;
}
...
void CPEN211DAGToDAGISel::Select(SDNode *Node) {
  ...
  default:
    break;
    // This is a way to select FrameIndex
    // The actual logic will be parsed in eliminateFrameIndex it seems
  case ISD::FrameIndex: {
